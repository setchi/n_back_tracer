...
public class PatternGenerator : MonoBehaviour {
	...
	public List<int> Generate(int length, List<int> ignorePattern) {
		...
		int[] fieldArr = SentinelsSetOfWall (new int[fieldSize], x, y);
	...
	bool DepthFirstSearch(int[] field, Vector2 current, int direction, ref Stack<int> stack) {
		if (stack.Count == 4)
			return true;
		...
		// 後ろは見ない
		int[] notBackIndexes = {0, 1, 3};
		for (int i = 0, l ; i < 2; i++) {
			int notBackIndex = notBackIndexes[Random.Range(0, notBackIndexes.Length - 1)];
			int newDirection = LoopIndex (direction + notBackIndex, directions.Length - 1);

			// 進めるところまで進む
			if (DepthFirstSearch (
				field,
				current + directions[newDirection],
				newDirection,
				ref stack
			)) {
				return true;
	...
	int[] SentinelsSetOfWall(int[] arr, int endX, int endY) {
		for (int y = 0; y < endY; y++)
			for (int x = 0; x < endX; x++)
				arr[y * endX + x] = (
					x == 0 ||
